...
12. Scheduled competitions require competition start time and duration, with registration timing mode support: `default` keeps registration open until start, while `manual` uses explicit registration start/end windows. Scheduled competitions allow exactly one attempt and transition from `published` to `live` through a trusted server-side start action at the server-authoritative start boundary, including the cron-driven scheduled-start worker path. Lifecycle transition `live` or `paused` to `ended` is owned by trusted competition lifecycle handlers. Scheduled competitions may end through the system timer at the server-authoritative end boundary or through organizer manual end from participant monitoring with explicit reason and `request_idempotency_token`; admin live-support controls do not include manual end.
...
18. Organizers configure anti-cheat behavior including question shuffling, option shuffling, tab-switch logging, offense-tier penalties, and optional Safe Exam Browser enforcement. SEB defaults to `off`. When an organizer sets SEB to `required`, every participant must open that quiz through the downloadable quiz-specific `.seb` config, and attempt start is blocked unless the request carries an approved SEB Config Key hash.
19. Organizers can download the quiz-specific SEB config after saving the draft, open it in the official SEB Config Tool, save it, copy the resulting 64-character Config Key hash, and paste that hash back into MathWiz before publishing. Organizers should keep the generated defaults unless they have tested changed SEB settings; any SEB Config Tool edit changes the Config Key hash and requires updating MathWiz.
20. Publish remains unavailable until the full wizard validates successfully.
21. Organizers can pause only open competitions so active attempts may finish while new attempts cannot start, resume paused owned competitions (including admin force-paused states), monitor live scheduled competitions, broadcast announcements with canonical audience predicates, extend competitions through trusted controls only when the competition status is `live` or `paused`, and reset attempts only when disconnect-reset criteria pass in fixed order: required non-empty `reason` plus `request_idempotency_token`, valid evidence taxonomy with same-attempt evidence reference, `in_progress` attempt state, evidence observed within 120 seconds, and no approved reset for the same attempt where `happened_at > server_now_at_request - interval '10 minutes'` (first failing gate determines rejected outcome). Pause, resume, extend, reset, and manual end controls require explicit reasons. Manual end for open and scheduled competitions requires explicit reason plus `request_idempotency_token`.
22. Open competitions can be retired only through a trusted archive path after active attempts have finished; hard delete is allowed only for `draft` competitions through trusted draft-delete controls. Non-draft hard delete is admin-only abuse or fraud moderation with mandatory audit evidence.
23. After competition completion, organizers review disputes, accept or reject them with resolution notes, recalculate scores if answer keys were wrong, publish scheduled leaderboards through an explicit organizer publish action, and export result data with participant/team context from immutable registration snapshots.
...
- Trusted lifecycle end transition (`live` or `paused` to `ended`) supports scheduled system-timer end (`transition_source = 'system_timer'`) and organizer manual end for open or scheduled competitions (`transition_source = 'trusted_manual_action'`) with required reason and `request_idempotency_token`; all end transitions must be idempotent, and admin live-support controls do not include manual end.
...
- Published competitions must preserve immutable problem and scoring snapshots, including Safe Exam Browser mode and approved Config Key hashes.
...
- Trigger-source mapping: scheduled competitions use `system_timer` at the effective server boundary or `trusted_manual_action` from authorized organizer participant monitoring controls; open manual end uses `trusted_manual_action` by authorized organizer controls only.
- Idempotency contract: replay with the same request token must return the existing terminal result and must not duplicate side effects; manual end requires explicit reason plus `request_idempotency_token`.
...
16. Before entering the arena, mathletes must accept the organizer rules and anti-cheat acknowledgement, including device-responsibility warnings. If SEB is required, the pre-entry UI must show the quiz-specific SEB config download and participants must start from inside SEB.
