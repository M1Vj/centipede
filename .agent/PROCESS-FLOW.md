...
12. Scheduled competitions require competition start time and duration, with registration timing mode support: `default` keeps registration open until start, while `manual` uses explicit registration start/end windows. Scheduled competitions allow exactly one attempt and transition from `published` to `live` through a trusted server-side start action at the server-authoritative start boundary. Lifecycle transition `live` or `paused` to `ended` is owned by trusted competition lifecycle handlers and is system-timer-owned only at the server-authoritative end boundary, with idempotent replay behavior. Organizer manual end is not allowed for scheduled competitions, and admin live-support controls do not include manual end.
