...
- 2026-04-23: Hardened organizer competition read compatibility so post-mutation refreshes fall back to legacy `competitions` select columns even when the modern read returns `data = null`, preventing successful create/publish/delete flows from surfacing generic `operation_failed` responses in under-migrated environments.
