...
| `safe_exam_browser_mode` | text | `off` or `required`; default `off` |
| `safe_exam_browser_config_key_hashes` | text[] | approved 64-character SEB Config Key hashes used to verify required SEB attempt starts |
...
- `end_competition(competition_id, reason, request_idempotency_token, transition_source)` trusted lifecycle transition from `live` or `paused` to `ended`; `transition_source` must be `system_timer` or `trusted_manual_action` with split enforcement: scheduled competitions accept `system_timer` for server boundary end and `trusted_manual_action` for organizer manual end from participant monitoring, while open manual end accepts organizer-only `trusted_manual_action` with required non-empty reason and `request_idempotency_token`; repeated requests are idempotent and return existing terminal state
...
- `submit_competition_attempt(attempt_id, actor_user_id, request_idempotency_token, submission_kind)` canonical final-submit mutation with deterministic authority and concurrency guards for individual and team registrations; `submission_kind = 'auto'` is the timer-expiry path. Successful first submit closes active intervals, locks the attempt as `submitted` or `auto_submitted`, and immediately calls `grade_attempt(attempt_id)`; duplicate terminal submits return existing final state and must not re-grade. Branch `13` does not write `leaderboard_entries` directly.
- `get_attempt_review_summary(attempt_id, actor_user_id)` derives review counts from persisted `attempt_answers.status_flag` rows and infers older missing answer rows as `blank` using `competition_problems` count minus distinct answer rows.
- `create_problem_dispute(competition_id, competition_problem_id, attempt_id, reporter_id, reason)` enforces post-end timing, same-attempt problem scope, ownership scope, duplicate open/reviewing replay, and deterministic anti-spam limits (branch `13-review-submission` ownership contract)
- `can_view_answer_key(competition_id, viewer_profile_id)` enforces `answer_key_visibility` (`after_end` requires ended or archived competition plus owned registration/attempt context and trusted end checks; `hidden` always denies participant access), independent from leaderboard publication
- `get_answer_key_snapshots(competition_id, viewer_profile_id)` returns only `competition_problems` snapshot answer key and explanation fields after `can_view_answer_key` allows access
...
- Safe Exam Browser attempt-start gate is an application-layer trusted route guard. When `competitions.safe_exam_browser_mode = 'required'`, `/api/mathlete/competition/{competition_id}/start` verifies `X-SafeExamBrowser-ConfigKeyHash` by hashing the absolute request URL without fragment plus each approved stored Config Key hash and comparing against the request header; missing or mismatched proof blocks attempt creation.
...
- Competition end-transition policy: `live` or `paused` to `ended` is owned by trusted lifecycle handlers with deterministic split rules: scheduled competitions end through `transition_source = 'system_timer'` at the server boundary or organizer `transition_source = 'trusted_manual_action'` from participant monitoring, while open competitions may end through organizer-only `transition_source = 'trusted_manual_action'`; admin live-support controls do not include manual end; end transitions are idempotent and must not duplicate state effects.
...
- Safe Exam Browser policy: default is `off`; required mode is quiz-scoped and mandatory for every participant. Organizer-owned config download may be available during draft setup; participant config download is exposed only for visible non-draft competitions where SEB is required.
...
- Competition-control reason policy: pause, resume, extend, and disconnect-reset control actions require a non-empty reason; organizer manual end requires a non-empty reason as well.
