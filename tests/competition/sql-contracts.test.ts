...
const REMOVE_ANTI_CHEAT_LOGGING_MIGRATION_PATH = join(
  process.cwd(),
  "supabase/migrations/20260601150000_remove_anti_cheat_logging.sql",
);
...
describe("competition sql contracts", () => {
  ...
  test("removed tab-switch logging drops server-side logging artifacts", () => {
    const sql = readFileSync(REMOVE_ANTI_CHEAT_LOGGING_MIGRATION_PATH, "utf8");

    expect(sql).toContain("drop function if exists public.log_tab_switch_offense(uuid, jsonb)");
    expect(sql).toContain("drop function if exists public.log_tab_switch_offense(uuid, jsonb, uuid)");
    expect(sql).toContain("drop table if exists public.tab_switch_logs");
    expect(sql).toContain("log_tab_switch = false");
    expect(sql).toContain("offense_penalties = '[]'::jsonb");
  });
