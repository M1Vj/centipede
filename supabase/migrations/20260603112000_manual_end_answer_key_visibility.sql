-- Manual organizer end should satisfy "after end" answer-key visibility even
-- when the scheduled end_time remains in the future.
...
begin
  ...
  if v_competition.status in ('ended'::public.competition_status, 'archived'::public.competition_status) then
    return true;
  end if;
