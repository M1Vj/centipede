-- ============================================================================
-- Forward fix: qualify competitions status return in publish/delete lifecycle RPCs
-- ============================================================================
...
begin
  ...
  set status = 'published'::public.competition_status,
      scoring_snapshot_json = coalesce(
        c.scoring_snapshot_json,
        jsonb_build_object(
          'scoring_mode', c.scoring_mode,
          'custom_points', c.custom_points,
          'penalty_mode', c.penalty_mode,
          'deduction_value', c.deduction_value,
          'tie_breaker', c.tie_breaker,
          'multi_attempt_grading_mode', c.multi_attempt_grading_mode,
          'attempts_allowed', c.attempts_allowed,
          'shuffle_questions', c.shuffle_questions,
          'shuffle_options', c.shuffle_options,
          'log_tab_switch', c.log_tab_switch,
          'offense_penalties', c.offense_penalties,
          'answer_key_visibility', c.answer_key_visibility
        )
      ),
      published_at = coalesce(c.published_at, timezone('utc', now())),
...
begin
  ...
  from public.competition_events ce
  where ce.competition_id = p_competition_id
    and ce.control_action = 'delete_draft_competition'
    and coalesce(ce.actor_user_id, '00000000-0000-0000-0000-000000000000'::uuid)
      = coalesce(v_actor_user_id, '00000000-0000-0000-0000-000000000000'::uuid)
    and ce.request_idempotency_token = v_token
  order by ce.happened_at desc, ce.id desc
