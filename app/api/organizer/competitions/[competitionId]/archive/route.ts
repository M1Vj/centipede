...
export async function POST(request: Request, context: { params: Promise<{ competitionId: string }> }) {
    ...
    return jsonOk({
      ...
      competition:
        "response" in refreshed
          ? withCompetitionStatus(competition, "archived")
          : refreshed.competition.status === "archived"
            ? refreshed.competition
            : withCompetitionStatus(refreshed.competition, "archived"),
      lifecycle: {
        machineCode: "ok",
        status: "archived",
        eventId: null,
  ...
  const archivedCompetition = withCompetitionStatus(competition, "archived");
