...
import { fetchActiveMembership, getTeamRosterLock, isTeamLeader } from "@/lib/teams/guards";
...
export async function DELETE(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const leader = await isTeamLeader(admin, teamId, actor.userId);
    if (!leader) {
      return jsonError("forbidden", "Only team leaders can remove members.", 403);
    }

    if (memberId === actor.userId) {
      return jsonError("invalid_request", "Use the leave action to exit a team.", 400);
    }
